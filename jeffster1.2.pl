...
do {
	do {
		...
		# FIX THIS SHIT #333333"><B><B>White Stripes&n
		&deleteArtist;
		print "$artist\n";
	} # do while loop to make sure artists exist
	until $artist2 =~ /$artist/i;
	...
	print DEBUG "made directory $artistDirectoryName\n";

	# for all albums
	for ($albumTitleIndex = 0; $albumTitleIndex < $numberOfAlbums; $albumTitleIndex++) {
		$albumName = $albumArray[2][$albumTitleIndex];
		$albumName =~ tr/[\/:\*\?\"|]/\-/;
		$albumDirectoryName = $artistDirectoryName . $albumName;
		print DEBUG "$albumDirectoryName\n";
		@playList = ();
		$playListIndex = 0;
		$playListIndex2 = 0;
		$songCount = 0;
		print "$albumDirectoryName\n";
		print DEBUG "$albumDirectoryName - $albumArray[2][$albumTitleIndex]\n";
		while ($albumArray[$playListIndex+3][$albumTitleIndex] ne "") {
			$songName = $albumArray[$playListIndex+3][$albumTitleIndex];
			$oldMP3 = $MP3Title;
			&enqueueSong;
			if ($oldMP3 ne $MP3Title) {
				$playList[$playListIndex2] = $MP3Title;
				$playListIndex2++;
			}
			$playListIndex++;
		} # while for each song in album
		
		# restart the queue if it starts to lag
		if ($songID % 5 == 0) {
			$URL = "http://www.audiogalaxy.com/satellite/queue.php?SID=" . $SID;
			$_ = get($URL);
			#print "jumpstarting queue\n";
		}
		
		# check if album is complete
		print "\n";
		if ($songCount != $playListIndex) {
			print "$songCount of $playListIndex\n";
			print DEBUG "songcount is now $songCount, and out of $playListIndex songs\n";
			$albumDirectoryName = $albumDirectoryName . " ($songCount of $playListIndex)";
			print DEBUG "album directory name set to $albumDirectoryName\n";
		} # end if
			
		if ($#playList > 2) {
			$playlistOutput = ">" . $albumDirectoryName . "\\" . $albumArray[2][$albumTitleIndex] . ".m3u";
			mkdir($albumDirectoryName . "\\", 0777);
			print DEBUG "made directory $albumDirectoryName\n";
			open(PLAYLIST, $playlistOutput);
			foreach $temp (@playList) {
				print PLAYLIST $temp . "\n";
			}
			close(PLAYLIST);
			print DEBUG "wrote playlist\n";
			&addToQueue;
		}
	# get number of enqueued songs
	$URL = "http://www.audiogalaxy.com/satellite/queue.php?SID=" . $SID;
	$_ = get($URL);
	/(\d+)&nbsp;transferring/g;
